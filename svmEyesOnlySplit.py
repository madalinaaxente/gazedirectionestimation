...
directions = ("Centre", "Up Right", "Up Left", "Right", "Left", "Down Right", "Down Left")
	...
	for folder, label in gazeDirections :
		...
		for filename in Path(path).glob('*.jpg'):
			...
			with open(imgFeaturesPath,'rb') as f:
				...
				#pdb.set_trace()
				estDir = directions[clf.predict(featuresImg.reshape(1,-1))[0]]
				cv2.putText(clone, estDir, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)		
				cv2.imshow("Image estimation", clone)
				cv2.waitKey(0)
