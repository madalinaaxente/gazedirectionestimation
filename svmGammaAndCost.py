...
with open('featuresData.pkl', 'rb') as f:
...
bestRate = 0
#pdb.set_trace()
workbook = xlsxwriter.Workbook('gammaAndCostPow2.xlsx' )
worksheet = workbook.add_worksheet()

for gam in range(-10,3):
	#pdb.set_trace()
	worksheet.write(chr(gam+76)+'1', '2^'+str(gam))
for c in range(-2,16):
	#pdb.set_trace()
	worksheet.write('A'+str(7+c), '2^'+str(c))


for gam in range(-10,3):
	for c in range(-2,16):
		clf = svm.SVC(gamma = 2**gam, C = 2**c)
		confMatrix = np.zeros((7,7))
		#pdb.set_trace()
		test = 1
		while test <38:
			...
			for folder, label in gazeDirections :
				for j in range(1, 38):
					...
					for filename in Path(path).glob('*.jpg'):
						imgFeaturesPath = os.path.splitext(filename)[0] + '.pkl'
